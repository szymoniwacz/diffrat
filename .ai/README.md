...
- Whole-project automation: create a **Project Execution** issue and authorize
  it with `/execute-project`. Goal Executor waits for PR CI (or resumes on
  PR-head CI completion); merged PRs continue automatically; green post-merge
  CI resumes the next goal, red CI repairs first. Use `/continue-project` only
  when needed. Runtime: `.ai/automation/project-executor.md`.
